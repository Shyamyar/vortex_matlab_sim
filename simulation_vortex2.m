...
%         yaw_pid = 0; % no yaw control
        ...
        T1 = min(max(T_des(alpha1) + h_pid - pitch_pid, 0), 0.7*g);
        T2 = min(max(T_des(alpha2) + h_pid + pitch_pid - roll_pid, 0), 0.7*g);
        T3 = min(max(T_des(alpha3) + h_pid + pitch_pid + roll_pid, 0), 0.7*g);
...
%         W3 = -yaw_pid2;
    ...
    elseif mode == 2
        alpha_c = deg2rad(55) * ones(3,1);
        T1 = min(max(T_des(alpha1) + h_pid - pitch_pid * cos(psi), 0), 0.7*g);
        T2 = min(max(T_des(alpha2) + h_pid + pitch_pid * cos(psi) - ...
                roll_pid * cos(psi), 0), 0.7*g);
